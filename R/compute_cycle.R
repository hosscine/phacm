...
bootstrap.homology <- function(X,maxdim,maxscale,const.band=0,maximum.thresh = F){
  ...
  for (t in 1:length(X)) {
    land <- lapply(1:maxdim,function(d) TDA::landscape(diags[[t]][[1]],dimension = d,KK = 1,tseq = tseq))
...
calc.landscape.peak <- function(X = diagram,dimension = 1,spar = seq(0,1,0.1),thresh = 0,tseq = tseq,show = F){
  ...
  smoothed.land <- lapply(spar, function(sp){stats::smooth.spline(x=tseq,y=X,spar=sp)})
