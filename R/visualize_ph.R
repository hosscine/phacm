...
rawdiag <- function(diag) {
  if (class(diag) == "list") 
    return(diag$diagram) else return(diag)
}
...
showPersistentDiagram <- function(diag = diagram, maxdimention = NA, scale = NA, point = 1.5) {
  ...
  if (missing(scale)) 
    scale <- attr(diag, "scale")
  maxdimention <- maxdimention + 1
  diag[is.infinite(diag)] <- scale[2]
  
  graphics::plot(diag[, 2], diag[, 3], xlim = scale, ylim = scale, cex = point, cex.axis = point, 
    col = diag[, 1] + 1, pch = diag[, 1] + 1, xlab = "Birth", ylab = "Death", cex.lab = point, 
    cex.main = 2)
  graphics::abline(0, 1)
